...
class OptimizedPositionAnalyzer:
    ...
    def run_optimized_analysis(self, signals_file, start_from=0, batch_size=50):
        """Run optimized analysis with better progress tracking"""
        print("=== OPTIMIZED POSITION ANALYSIS ===")
        ...
        print(f"Starting from signal: {start_from}")
        print(f"Batch size: {batch_size}")
        
        # Process in batches
        for start_idx in range(start_from, total_signals, batch_size):
            ...
            if batch_results:
                ...
                batch_file = f'position_analysis_batch_{batch_num:03d}.csv'
                batch_df_result.to_csv(batch_file, index=False)
                print(f"  Batch {batch_num} saved: {batch_file} ({len(batch_results)} signals)")
            ...
            progress_file = f'analysis_progress_checkpoint.txt'
            with open(progress_file, 'w') as f:
                f.write(f"Last processed batch: {batch_num}\n")
                f.write(f"Last processed signal: {end_idx}\n")
        ...
        if self.results:
            ...
            results_df.to_csv('complete_position_analysis.csv', index=False)
            print(f"\n=== ANALYSIS COMPLETE ===")
            print(f"Total signals analyzed: {len(results_df)}")
            print(f"Final results saved to 'complete_position_analysis.csv'")
...
if __name__ == "__main__":
    analyzer = OptimizedPositionAnalyzer()
    
    # Check if there's a progress checkpoint
    if os.path.exists('analysis_progress_checkpoint.txt'):
        with open('analysis_progress_checkpoint.txt', 'r') as f:
            lines = f.readlines()
            last_signal = int(lines[1].split(': ')[1])
        print(f"Resuming from signal {last_signal + 1}")
        results = analyzer.run_optimized_analysis('Backtest Monthly HA and MACD', start_from=last_signal + 1)
    else:
        results = analyzer.run_optimized_analysis('Backtest Monthly HA and MACD')
